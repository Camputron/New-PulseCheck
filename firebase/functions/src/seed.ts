  ...
  },
  {
    ...
    questions: [
      ...
      },
      {
        id: "seed-q-bio-3",
        prompt_type: "multi-select" as const,
        prompt: "Which of the following are found in plant cells but NOT animal cells?",
        prompt_img: null,
        points: 10,
        anonymous: false,
        time: 30,
...
async function seedPolls() {
  ...
  for (const poll of POLLS) {
    const questionRefs = poll.questions.map((q) =>
      questionRef(poll.id, q.id)
    )
    ...
    for (const s of students) {
      ...
      respBatch.set(
        db.doc(`sessions/${sid}/questions/${sqid}/responses/${s.id}`),
        {
          ...
          choices: [
            db.doc(
              `sessions/${sid}/questions/${sqid}/options/${chosen.id}`
            ),
          ],
          correct: isCorrect,
          created_at: ts(),
...
async function clean() {
  ...
  await Promise.all(POLLS.map((p) => db.doc(`polls/${p.id}`).delete().catch(() => {})))
  ...
  for (const sid of sessionIds) {
    ...
    await db.doc(`sessions/${sid}`).delete().catch(() => {})
  }

  // Delete seed submissions, users, and auth accounts
  for (const u of USERS) {
    await db.doc(`submissions/seed-sub-fin-${u.id}`).delete().catch(() => {})
    await db.doc(`users/${u.id}`).delete().catch(() => {})
